...
1. Every predicate the engine attaches to the target node (the shapes in VIRTUAL_CYPHER §7.6.1) is part of the **fetch's cache key** — and attachment does not stop at a `WITH`: `MATCH …->(run) WITH run WHERE run.status = 'completed'` is attached exactly as if it were in the MATCH's own WHERE. So is a `LIMIT` on the final `RETURN`. Two views over the same door whose attached sets differ re-read the source separately, even inside the TTL; the same view with a different `LIMIT` re-reads too. To make several views share ONE cached fetch, filter on **projected variables**, not on the node: `WITH run, run.created_at AS createdAt, run.status AS status WHERE createdAt >= $since AND status = 'completed'` attaches nothing; and put a `LIMIT` on a `WITH` (`WITH run, createdAt ORDER BY createdAt DESC LIMIT $n`), never on the `RETURN`. A narrowing `WITH … LIMIT` must carry the order key it sorts by, or the anchor count for the next hop is not reduced and the hop is refused as too wide. This is the pattern for a `periods:` door (VIRTUAL_CYPHER §5.17), where the fetch is the expensive thing and the filtering is free; measured, it took a 12-view app from 91 s to a few seconds.
